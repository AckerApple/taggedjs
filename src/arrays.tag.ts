...
export const arrays = tag(() => {/* ArrayTests */
  ...
  return div(
    span(
      'count display:',
      span.id`arrays-counter-display`(_=> counter)
    ),

    fieldset(
      legend(
        'signal array test ',
        sup.id`signal-array-count`(_=> signalArray.length)
      ),
      div.style`display:flex;flex-wrap:wrap;gap:1em`(subscribe(signalArray, array => {
          ...
          button.type`button`.id`push-signal-array-btn`.onClick(() => addArraySignal(1))('add number'),

          button.type`button`.onClick(() => addArraySignal(10))('add 10 number'),
    ...
    fieldset.style`display:flex;flex-wrap:wrap;gap:1em`(legend('simple array test - length:', _=> simpleArray.length), _=> simpleArray.map((x, index) =>
        div(
          'counter:', _=> counter,
          ' index:', _=> index,
          ' x:', _=> x,
    ...
    fieldset(
      ...
      div.style`display:flex;flex-wrap:wrap;gap:1em`(div('players count:', _=> {
      ...
      _=> players.length > 0 &&
          button.attr(fxValue as any).onClick(() => players.length = 0)(`remove all ${players.length} players`),
    ),
...
const getPlayerDisplay = tag((
  ...
  return div.style`background-color:black;`
    .attr(fx({duration: '.1s'}) as any)(div(_ => `name:${player.name}`), div(_ => {
      return `index:${index}`
    }), div.style`background-color:purple;padding:.5em`('scores:', _ => player.scores.map((score, playerIndex) => {
        return playerScoreDisplay(playerIndex, score).key(score)
      })), _=> player.edit && button.onClick(() => {
        removePlayerByIndex(index)
      })('remove'), _=> player.edit && button.id(_=> `player-remove-promise-btn-${index}`).onClick(async () => {
        player.edit = !player.edit
        removePlayerByIndex(index)
        return 'player-remove-promise-btn-xx'
      })('remove by promise'), button.id(_ => `player-edit-btn-${index}`).onClick(() => player.edit = !player.edit)('edit'), button.onClick(() => {
...
const playerScoreDisplay = tag((
  ...
  return div.class`animate__slow`.attr(fx({duration: '.1s'}) as any)(fieldset(
      legend(
        button.id(_=> `score-data-${playerIndex}-${score.frame}-outside-button`).onClick(() => ++score.score)(_=> `outer score button ++${score.score}`),
        span.id(_=> `score-data-${playerIndex}-${score.frame}-outside-display`)(_=> score.score)
      ),
