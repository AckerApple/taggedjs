...
const outerHtml = tag((
  ...
  return div.id`outer-html-fx-test`
    .attr(fx({stagger: staggerBy, duration: '.1s'}) as any)
    .attr("style.border", `1px solid orange`)
    (xx)
})
...
export const concatStyles = tag((innerHTML: any) => {
  ...
  return div(
    ...
    div.id`dynamic-border-element`.style(_=> `border-width:${width}px;border-color:${borderColor};border-style:solid;`)(innerHTML),

    div(
      'borderWidth:',
      input.id`dynamic-border-width`.type`range`.min`0`.max`10`.step`1`.value(_=> width).onChange(event => width = Number(event.target.value))(),
      ' - ',
      _=> width,
      'px'
    ),
...
export const content = tag(() => {
  ...
  return [
    ...
    fieldset.style`flex-grow:1`(legend(
      ...
      ), pipe, button.type`button`.onClick(() => ++counter)('increase outside ', _=> counter)),
    ...
    fieldset.style`flex-grow:1`(
      ...
      _=> showHideFx && div.name`test-the-tester`
        .attr(fx({duration:'10ms'}) as any)(
          'test the tester - 0'
        ),
        _=> showHideFx && div
          .name`test-the-tester`
          .attr(fx({duration:'10ms', stagger: staggerBy}) as any)(
            'test the tester - 1'
          ),
        _=> showHideFx && div
          .name`test-the-tester`
        ...
        div(
          'added: ',
          span.id`content-fx-added`(subscribe(counts, counts => counts.added)),
          ' ',
          'removed: ',
          span.id`content-fx-removed`(subscribe(counts, counts => counts.removed))
        ),
        div(
          'staggerBy:',
          input.type`range`.min`10`.max`300`.step`1`.onChange(event => staggerBy = Number(event.target.value))()
        )
    ...
    div.style`display:flex;flex-wrap:wrap;gap:1em;`(
      ...
      div.id`style-simple-border-orange`.attr("style.border", `3px solid orange`)(
        'simple orange border'
      ),
      div.id`style-var-border-orange`.attr("style.border", `3px solid orange`)(
        'var orange border'
      ),
      div(
        div.id`style-toggle-border-orange`.attr("style.border", _=> orangeToggle ? "3px solid orange" : "3px solid green")(
          'toggle orange border'
        ),

        button.id`toggle-border-orange`.onClick(() => orangeToggle = !orangeToggle)('orange toggle ', _=> orangeToggle),
      ), div(
        div.id`style-toggle-bold`.style(_=> boldToggle ? 'font-weight:bold;' : '')('toggle orange border'),

        button.id`toggle-bold`.onClick(() => boldToggle = !boldToggle)('bold toggle ', boldToggle ? 'true' : 'false'),
      ), div.id`hello-spacing-dom-world`(54, ' hello', ' worlds')),

    div.style`display:flex;flex-wrap:wrap;gap:1em;font-size:0.8em`(div.style`flex-grow:1`(
      fieldset(
          legend('zero test'),
      ...
      div.style`flex-grow:1`(
        ...
        div.style`flex-grow:1`('P.5 You should see "{22}" here => "', `${'{'}`, '22', `${'}'}`, '"')
      ), fieldset(
        div.style`flex-grow:1`('P.6 You should see "{__tagVar0}" here => "{__tagVar0}"')
      ), div.style`flex-grow:1`('should be a safe string no html ', span.id`content-dom-parse-0-0`('"<div>hello</div>"'), ' here => ', span.id`content-dom-parse-0-1`(`"${'<div>hello</div>'}"`))),
