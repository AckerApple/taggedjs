import { tag, fieldset, legend, label, input, div, select, option, h3, span, host, getContextInCycle } from "taggedjs";
...
export const inCycleContextComms = tag(() => {
  ...
  const test = () => {
    const x = inCycleParent(cycleColorParent)
    return x
  }

  const test2 = () => {
    return inCycleChild(cycleColorChild)
  }

  return fieldset.id`in-cycle-context-comms`(
    legend(
      label(
        input.type`checkbox`.checked(_ => !hideShowCycles).onChange(e => hideShowCycles = !hideShowCycles)(),
        ...
        div.id`fieldset-body-wrap`.style`margin-bottom: 1em`(
          label(
            ...
            select.id`parent-color-select`.onChange(e => cycleColorParent = e.target.value)(
              _ => colorOptions.map(color => option.value(color).selected(_ => cycleColorParent === color)(color).key(color)
              )
            )
          ),
          ...
          label.style`margin-left: 1em`(
            ...
            select.id`child-color-select`.onChange(e => cycleColorChild = e.target.value)(
              _ => colorOptions.map(color => option.value(color).selected(_ => cycleColorChild === color)(color).key(color)
            )
            )
          ),
          label.style`margin-left: 1em`(
