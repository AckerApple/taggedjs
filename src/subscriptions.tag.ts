...
export const subscriptions = tag(() => {
  ...
  return div(
    ...
    fieldset.id`noParentTagFieldset`(legend('Pass subscription'), _=> passSubscription({sub0, sub1}))
  )
})
...
const passSubscription = tag(({
  ...
  return noElement(
    ...
    button.id`passed-in-sub-increase`.onClick(() => sub0.next((sub0.value || 0) + 1))('sub0 increase'),

    button.id`passed-in-sub-next`.onClick(() => ob.next(sub0.value = (sub0.value || 0) + 1))('ob increase'),

    button.id`passed-in-sub-hide-show`.onClick(() => onOff = !onOff)('hide/show on/off = ', _=> onOff ? 'show' : 'hide'),
    ...
    div(
      strong('test 0'),
      div.id`passed-in-sub-ex0`('0||', _=> onOff && subscribe(sub0), '||0')
    ),

    div(
      strong('test 1'),
      div.id`passed-in-sub-ex1`('1||', _=> onOff && subscribe(sub0, numberFun), '||1')
    ),

    div.id`passed-in-sub-ex2`('2||', _=> onOff && subscribe(sub0, numberTag), '||2'),

    div.id`passed-in-sub-ex3`('3||', subscribe(sub1, numberTag), '||3'),

    div.id`passed-in-sub-ex4`('4||', subscribe(ob, numberTag), '||4'),

    div.id`passed-in-sub-ex4`('5||', subscribe(ob.pipe( startWith(33) ), numberTag), '||5'),

    div.id`passed-in-sub-ex4`('6||', subscribe(
        ob.pipe( startWith(undefined) ),
        (x: number) => numberTag(x)
      ), '||6'),

    div.id`passed-in-sub-ex4`('7||', subscribe(ob, (x: number) => numberTag(x)), '||7')
  )
})
...
const testHost = tag(() => {
  ...
  return fieldset.style`flex-grow:1`(legend('host'), _=> hideShow && noElement(
      span.id`hostedContent`.attr(host(
      ...
      button.type`button`.onClick(() => ++clickCounter)('clickCounter:', _=> clickCounter)
    ), button.id`hostHideShow`.onClick(() => hideShow = !hideShow)('hide/show'), div(
      'destroyCount: ',
