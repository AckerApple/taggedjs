...
//  watch.noInit([showAll], ([showAll]) => showLower = showAll)
  ...
  const getHead = (allowMaximize?: boolean) => {
    return div.class`taggedjs-object-label`.style(_=> showLower ? 'border-bottom-width:1px;border-bottom-style:solid;border-color:black;' : '')(
      a.onClick(() => {
        ...
        sup.style`opacity:80%;font-size:75%;padding-left:0.4em`(
          '{', _=> Object.keys(value as any).length, '}',
        )
      ),
  ...
  const getDumpBody = (allowMaximize?: boolean) => {
    return div.class`taggedjs-object-body-wrap`(
      _=> Object.entries(value as any).map(([key, value]) => 
        /* recurse */
        div.class`taggedjs-object`.style(_=> !value || typeof(value) !== 'object' ? 'flex: 1 1 10em;' : 'flex-grow:1;')(
          _=> dump({
            value,
            key,
            show: showLower,
  ...
  return div.style`flex: 1 1 10em;text-align:left;`(
    div.class`taggedjs-object-wrap`(
      ...
      dialog
        ...
        .attr(
          'onmousedown',
          `var r = this.getBoundingClientRect();(r.top<=event.clientY&&event.clientY<=r.top+r.height&&r.left<=event.clientX&&event.clientX<=r.left+r.width) || this.close()`
        )(
        div.style`padding:.25em`.attr('onmousedown', `this.parentNode.draggable=true`)(
          _=> maximize && getHead(false),
        ),
      
        _=> maximize && getDumpBody(false),
