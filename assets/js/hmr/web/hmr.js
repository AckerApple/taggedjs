...
import { ValueTypes } from "taggedjs";
...
async function updateByElement(app) {
    ...
    tags.forEach(async (tag) => {
        ...
        oldSetUse.tagUse.push(...newSetUse.tagUse);
        ...
        if (tagChangeStates.length) {
            ...
            const count = await replaceTemplater(lastApp, tagChangeStates[0], hmr);
 ...
 */
async function replaceTemplater(ownerSupport, { oldTag, newTag }, hmr) {
    let count = 0;
    const tag = ownerSupport.templater.tag;
    const promises = tag.values.map(async (value, index) => {
        ...
        if (isWrapping) {
            ...
            updateSubject(contextSubject, newTag, oldTag, hmr);
            ++count;
            return;
        }
    });
    ...
    const subPromises = context.map(async (child) => {
        ...
        const support = childGlobal.newest;
        if (support) {
            count = count + await replaceTemplater(support, { oldTag, newTag }, hmr);
        }
    });
...
function wrapperHasTagMethod(wrapper, oldTag) {
