import { ValueTypes, paint } from "taggedjs";
 ...
 */
export async function replaceTemplater(ownerSupport, { oldTag, newTag }, hmr, isApp) {
    if (!newTag) {
    ...
    const subPromises = context.map(async (child) => {
        ...
        if (support) {
            ...
            if (newCount > 0) {
                ...
                if (isApp && oldTag.original?.isApp) {
...
async function checkToUpdateSubject(value, contextItem, oldTag, newTag, hmr, count, isApp) {
    ...
    if (oldTag.original.toString() === newOriginal?.toString()) {
        ...
        await updateSubject(contextItem, newTag, oldTag, hmr);
        ++count;
    }
    return count;
}
...
*/
//# sourceMappingURL=replaceTemplater.function.js.map