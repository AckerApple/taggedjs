...
export const app = tag(() => {
    ...
    return noElement(_ => router.route === '/info' ? infoTag : x, div.style `text-align:center;`(a({ href: "#/info" }, 'info'), '   ', a({ href: "#/" }, 'todo app')));
    ...
    const todosTag = tag((route) => {
        ...
        main.class `main`(_ => todos.length > 0 &&
            ...
            }), label({ class: "toggle-all-label", for: "toggle-all" }, 'Toggle All Input')), ul({ class: "todo-list show-priority" }, _ => visibleTodos.map((todo, index) => Item(todo, dispatch, index).key(todo.id)))));
    });
});
