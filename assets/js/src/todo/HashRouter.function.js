import { callback, onDestroy, state, ValueSubject } from "taggedjs";
const getHash = () => window.location.hash.substring(1) || '/';
/** @deprecated - only should run once */
...
export const useHashRouter = () => {
    ...
    const onHashChange = callback((route) => {
...
/** Hook into browser window hash changes and cause state to render after change */
export const hashRouterSubject = () => {
    const memory = {
        route: getHash(),
        location: window.location,
    };
    ...
    // runs function call only once
    const listener = HashRouter(onHashChange);
    onDestroy(() => window.removeEventListener('hashchange', listener));
    const subject = new ValueSubject(memory);
    return subject;
};
