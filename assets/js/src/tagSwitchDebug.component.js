import { tag, div, h3, button } from "taggedjs";
...
witch - dropdown `.onChange(changeSelectedTag)(option, option({
          value: "",
        ...
        ), testTag2, div({style:"border:1px solid blue;flex-grow:1"},
...
`
      ), () => renderCountDiv({renderCount, name:'tag1'})): {title: string}) => {
  let counter = 0
  let renderCount = 0
...
`), () => renderCountDiv({renderCount, name:'tag2'}))tag(({title}: {title: string}) => {
  let counter = 0
  let renderCount = 0
...
`), () => renderCountDiv({renderCount, name:'tag3'}))g = tag((
...
value `
      ...
      case '':
        ...
        return `;
space ` // tests how .previousSibling works
