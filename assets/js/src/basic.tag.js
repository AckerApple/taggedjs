...
export const basic = tag(() => {
    ...
    let showDiv = true;
    renderCount++;
    return div(h2('Basic Component'), p(_ => `Counter: ${counter}`), p(_ => `Render Count: ${renderCount}`), button.onClick(() => counter++)('Increment Counter'), button.onClick(() => showDiv = !showDiv)(_ => `Toggle Div (${showDiv ? 'Hide' : 'Show'})`), _ => showDiv && boltTag(counter));
});
const boltTag = tag((parentCounter) => {
    let clickCount = 0;
    let color = 'red';
    boltTag.updates(x => [parentCounter] = x);
    return div(() => `color: ${color}`, select.onChange(e => color = e.target.value)(option({ value: 'red' }, 'red'), option({ value: 'green' }, 'green')));
});
...
//# sourceMappingURL=basic.tag.js.map