...
export const arraysDisplay = tag(({ showLevels, showAll, showKids, array, arrayView, formatChange, allowMaximize, everySimpleValue, }) => {
    arrayDisplay.updates(x => {
        if (x[0].array != array || x[0].array.length != array.length) {
            allColumnNames = array.length ? getAllKeys(array) : [];
        }
    ...
    let allColumnNames = array.length ? getAllKeys(array) : [];
    ...
    return noElement(arrayTag, dialog
        .id(uniqueId)
        .class `dump-dialog`
        .style `padding:0`
        ...
        .attr('ondragend', "const {t,e,d}={t:this,e:event,d:this.drag};if (d.initX === d.x) {d.x=d.x+e.offsetX-(d.startX-d.x);d.y=d.y+e.offsetY-(d.startY-d.y);this.style.transform=translate3d(d.x+'px', d.y+'px', 0)};this.draggable=false")(div
        .style `padding:.25em;background-color:#666;color:white;`
        .attr('onmousedown', "this.parentNode.draggable=true")('Column Modifier'), div
        .style `padding:.25em`(_ => allColumnNames.map(name => {
        const included = columnNames === undefined || columnNames.includes(name);
        return div
            .style `display:flex;justify-content: space-between;flex-wrap:wrap`
            .class `hover-bg-warning`(_ => columnEditor({
            name,
            array,
            included,
