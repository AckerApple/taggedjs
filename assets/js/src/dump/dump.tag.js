...
let dumpCount = 0;
const getObjectTemplate = tag(({ value, showKids, key, onHeaderClick, everySimpleValue, format, isRootDump, showAll, showAllChange, formatChange, show, showLevels, allowMaximize, }) => {
    ...
    const getJsonDump = () => textarea.attr `disabled`.wrap `off`.style `width:100%;height:25vh;min-height:400px;color:white;background-color:black;`(_ => JSON.stringify(value, null, 2));
    return noElement(_ => {
        if (value === null) {
            if (!showKids) {
                return 'no kids';
