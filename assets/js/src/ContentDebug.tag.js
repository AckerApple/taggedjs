import { subscribeWith, html, tag, ValueSubject, state, combineLatest, willPromise, states, subscribe, Subject, getInnerHTML, host } from "taggedjs";
...
const animateWrap = (counts, staggerBy = testStaggerBy) => {
...
export const content = tag(() => {
    const sub0 = state(() => new Subject());
    const sub1 = state(() => new ValueSubject(3));
    const subArray = state(() => new ValueSubject(['a', 'b', 'c']));
    ...
    const counts = state(() => new Subject({ added: 0, removed: 0 }));
    states(get => [{
      ...
      <div style="display:flex;flex-wrap:wrap;gap:1em;font-size:0.8em">
        ...
        return array.map(x => html `<option value=${x}>${x}</option>`.key(x));
          ...
          <fieldset style="flex-grow:1">
            ...
            <span id="content-combineLatest-pipeHtml-display1">${subscribe(combineLatest([vs0, vs1]).pipe(willPromise(x => Promise.resolve(html `<b>bold 77</b>`))))}</span>
          </fieldset>
        </div>
      </div>
    </fieldset>
...
const passSubscription = tag(({ sub0, sub1, }) => {
    let onOff = false;
    // const ob = state(() => new Observable()) as any
    const ob = state(() => new RxSubject());
    ...
    <div id="passed-in-sub-ex4">6||${subscribe(ob.pipe(startWith(undefined)), (x) => numberTag(x))}||6</div>
    <div id="passed-in-sub-ex4">7||${subscribe(ob, (x) => numberTag(x))}||7</div>
  `;
});
const numberFun = (x) => {
    ...
    <fieldset style="flex-grow:1">
      ...
      ${hideShow && html `
        <span id="hostedContent"
          ${host(element => element.innerHTML = Date.now().toString(), {
...
//# sourceMappingURL=ContentDebug.tag.js.map