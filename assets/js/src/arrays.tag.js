...
export const arrays = tag(() => {
    ...
    ++renderCount;
    return div(span('count display:', span.id `arrays-counter-display`(_ => counter)), fieldset(legend('signal array test ', sup.id `signal-array-count`(_ => signalArray.length)), div.style `display:flex;flex-wrap:wrap;gap:1em`(subscribe(signalArray, array => {
        return array.map((x, index) => {
            return div({
                attr: arrayFx,
                id: _ => `signal-array-item-${index}`,
                style: "border:1px solid black;border-radius:.2em"
            }, 'index:', _ => index, ' counter:', span({ id: _ => `signal-array-item-counter-display-${index}` }, _ => counter), ' content:', _ => x, ' length:', _ => signalArray.length, button({
                id: `signal-array-item-delete-btn-${index}`,
                onClick: () => {
                    signalArray.splice(index, 1);
                }
            }, '🗑️ delete subscribe'), button({
                type: "button",
                onClick: () => ++counter
            }, '++counter ', _ => counter)).key(x);
        });
    }), div(button({
        type: "button",
        id: "signal-array-increase-counter",
        onClick: () => ++counter
    }, '++counter ', _ => counter), button({
        type: "button",
    ...
    array - test - push - item `.onClick(() => {
      ...
      _=> players.length > 0 &&
          button.attr(fxValue).onClick(() => players.length = 0)(`;
    remove;
    all;
    $;
    {
...
const getPlayerDisplay = tag((
  ...
  return div.style`;
    ...
    }, 'remove'), _=> player.edit && button({
      id: _=> `;
    player - remove - promise - btn - $;
    {
        index;
    }
    `,
    ...
    animate__slow `.attr(fx({duration: '.1s'}))(fieldset(
      legend(
        button({
          id: _=> `;
    score - data - $;
...
//# sourceMappingURL=arrays.tag.js.map