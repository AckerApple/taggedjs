...
export const injectionTag = tag(() => {
    ...
    const selectedTest = [];
    const wrapHost = injectionWrap(selectedTest, (newSelected) => {
        selectedTest.length = 0;
        selectedTest.push(...newSelected);
    });
    wrapHost.TEST = '👈👈👈👈';
    return div.style `padding: 20px;`(h2('Injection Test'), 'selected: ', _ => {
      ...
      `
    }, h3('Selected Items:'), p(_ => selectedTest.length > 0
        ? selectedTest.map(x => x.id).join(', ')
        : 'No items selected. Click to select items.')), div({ style: "margin-top: 10px; color: #666;" }, small(strong('Instructions:'), br, '- Click to select/deselect items', br, '- Click on empty space to deselect all', br, '- Selected items will be highlighted in green')));
});
//# sourceMappingURL=injectionTesting.tag.js.map