...
export const intervalTester0 = tag(() => {
    ...
    ++renderCounter;
    return div(div('interval type 1 at ', _ => test0interval, 'ms'), 'intervalId: ', _ => intervalId, button.type `button`.onClick(increase)(_ => intervalCount, ':', _ => renderCounter), input.type `range`.min `0`.max(_ => test0interval).step `1`.value(_ => currentTime)(), div('--', _ => currentTime, '--'), button.type `button`.onClick(toggle)["style.background-color"](intervalId || intervalId2 ? 'red' : 'green')('start/stop'), button.type `button`.onClick(delayIncrease)('delay increase currentTime'));
});
export const intervalTester1 = tag(() => {
    ...
    onDestroy(destroy);
    ++renderCounter;
    return div(div('interval type 2 with ', _ => test1interval, 'ms'), 'intervalId: ', _ => intervalId, button.type `button`.onClick(increase)(_ => intervalCount, ':', _ => renderCounter), input.type `range`.min `0`.max(_ => test1interval).step `1`.value(_ => currentTime)(), div('--', _ => currentTime, '--'), button.type `button`.onClick(toggleInterval)["style.background-color"](_ => intervalId ? 'red' : 'green')('start/stop'));
});
