...
export const App = tag(() => (_firstState = letState('app first state')(x => [_firstState, _firstState = x]), appCounter = letState(0)(x => [appCounter, appCounter = x]), toggleValue = letState(false)(x => [toggleValue, toggleValue = x]), toggle = () => toggleValue = !toggleValue, appCounterSubject = state(() => new Subject(appCounter)), renderCount = letState(0)(x => [renderCount, renderCount = x]), testTimeout = letState(null)(x => [testTimeout, testTimeout = x])) => {
    ...
    onInit(() => {
        ...
        runTesting(false);
        appCounterSubject.subscribe(callbacks(y => {
            appCounter = y;
        }));
    });
