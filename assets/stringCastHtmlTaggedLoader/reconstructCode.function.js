// taggedjs-no-compile
import { string } from "./stringCastHtmlTagged.function.js";
export function reconstructCode(parsedResults, funName = 'html') {
    const allStrings = [];
    ...
    const transform = (item) => {
        if (typeof item === string) {
            ...
            if (!transformedValues.length) {
                return `${funName}(allStrings[${currentStringsIndex}])`;
            }
            return `${funName}(allStrings[${currentStringsIndex}], ${transformedValues.join(', ')})`;
        }
    };
